...
JNI_FUNC(jint, PdfiumCore, nativeBuildBookMarkTree)(JNI_ARGS, jlong docPtr, jobject bmk) {
    if (bmkNode_add == 0) {
        jclass bmkNode = env->FindClass("com/ahmer/afzal/pdfium/bookmarks/BookMarkNode");
        bmkNode_addToParent = env->GetMethodID(bmkNode, "addToParent",
                                               "(Ljava/lang/String;I)Lcom/ahmer/afzal/pdfium/bookmarks/BookMarkNode;");
...
JNI_FUNC(jlong, PdfiumCore, nativeGetStringChars)(JNI_ARGS, jstring key, jboolean isCopy) {
    //LOGE("fatal nativeGetStringChars %ld", (long)key);
    return (long) env->GetStringChars(key, reinterpret_cast<jboolean *>(isCopy));
}
